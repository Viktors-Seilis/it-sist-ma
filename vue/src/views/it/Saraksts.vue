        ...
        >
          ...
          <b-table striped hover responsive="true" small v-b-hover fixed
            ...
            <template #cell(actions)="row">
              <b-button size="sm" variant="warning" @click="$router.push({ name: 'ItView', params: {id: row.item.id} })" class="mr-1 mt-2">Skatīt/Labot
              </b-button>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'departments', label: 'Nodaļa', sortable: true },
        { key: 'units', label: 'Atrašanās vieta', sortable: true },
  ...
  methods: {
    archiveItem(id, index) {
      axios.delete('/problems/'+id).then(response => {
    ...
    async getData() {
      this.isBusy = true;
      await axios.get('/problems?page[number]=').then(response => {
